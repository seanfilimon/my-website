          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredArticles.map((article) => (
              <Link
                ...
                className="group relative flex w-full flex-col gap-3 text-sm cursor-pointer select-none"
              >
                {/* Thumbnail */}
                <div className="relative aspect-video w-full overflow-hidden rounded-lg border hover:shadow-sm transition-all duration-300 bg-muted/20">
                  ...
                  <img
                    src={article.thumbnail || `/api/og/article?title=${encodeURIComponent(article.title)}&excerpt=${encodeURIComponent(article.excerpt || "")}`}
                    alt={article.title}
