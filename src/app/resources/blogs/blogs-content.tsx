...
export function BlogsContent({ blogs, categories }: BlogsContentProps) {
  ...
  const filteredBlogs = blogs.filter(blog => {
    ...
    const matchesSearch = searchTerm === "" || 
      ...
      (blog.excerpt?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
              ...
              {filteredBlogs.map((blog) => (
                ...
                >
                  ...
                  <div className="relative aspect-video w-full overflow-hidden rounded-lg border hover:shadow-sm transition-all duration-300 bg-muted/20">
                    ...
                    <img
                      src={blog.thumbnail || `/api/og/blog?title=${encodeURIComponent(blog.title)}&excerpt=${encodeURIComponent(blog.excerpt || "")}`}
                      alt={blog.title}
                    ...
                    {blog.category && (
                      <div className="absolute top-2 left-2 z-20">
                        <span 
                          ...
                          style={{ borderColor: blog.category.color || undefined }}
                        >
                          {blog.category.name}
                        </span>
                      </div>
